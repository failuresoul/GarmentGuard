...
BEGIN
  FOR r IN (SELECT table_name FROM user_tables WHERE table_name IN (
    ...
    'CERTIFICATION', 'AUDIT', 'WORKER', 'USER_', 'BUYER', 'FACTORY', 'ERROR_LOG', 'SAFETY_ALERT'
  )) LOOP
...
BEGIN
  FOR r IN (SELECT sequence_name FROM user_sequences WHERE sequence_name IN (
    ...
    'SAFETY_EQUIPMENT_SEQ', 'GRIEVANCE_SEQ', 'SALARY_RECORD_SEQ', 'BUYER_SEQ', 'ERROR_LOG_SEQ', 'SAFETY_ALERT_SEQ'
  )) LOOP
