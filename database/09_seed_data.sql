...
PROMPT Granting base table and view privileges to roles...
...
-- USERS (Includes bcrypt password hashes, worker_id, and buyer_id mappings)
INSERT INTO USER_ (user_id, username, password_hash, role, full_name, factory_id, worker_id, buyer_id, email, status)
VALUES (1, 'sysadmin', '$2b$10$6WyU3pjoR3da3dEHrWAYe.bHwqz52O/JcW53amQrrP1kkjbgSgC/2', 'Admin', 'Tariqul Islam', NULL, NULL, NULL, 'admin@garmentguard.org', 'Active');

INSERT INTO USER_ (user_id, username, password_hash, role, full_name, factory_id, worker_id, buyer_id, email, status)
VALUES (2, 'comp_dhaka', '$2b$10$6WyU3pjoR3da3dEHrWAYe.bHwqz52O/JcW53amQrrP1kkjbgSgC/2', 'Compliance_Officer', 'Rahman Khan', 1, NULL, NULL, 'rahman@dhakagarments.com', 'Active');

INSERT INTO USER_ (user_id, username, password_hash, role, full_name, factory_id, worker_id, buyer_id, email, status)
VALUES (3, 'comp_tongi', '$2b$10$6WyU3pjoR3da3dEHrWAYe.bHwqz52O/JcW53amQrrP1kkjbgSgC/2', 'Compliance_Officer', 'Nusrat Jahan', 3, NULL, NULL, 'nusrat@tongitex.com', 'Active');

INSERT INTO USER_ (user_id, username, password_hash, role, full_name, factory_id, worker_id, buyer_id, email, status)
VALUES (4, 'inspector1', '$2b$10$6WyU3pjoR3da3dEHrWAYe.bHwqz52O/JcW53amQrrP1kkjbgSgC/2', 'Inspector', 'Kazi Ashraful', NULL, NULL, NULL, 'kazi.a@garmentguard.org', 'Active');

INSERT INTO USER_ (user_id, username, password_hash, role, full_name, factory_id, worker_id, buyer_id, email, status)
VALUES (5, 'hm_buyer', '$2b$10$6WyU3pjoR3da3dEHrWAYe.bHwqz52O/JcW53amQrrP1kkjbgSgC/2', 'Buyer', 'Sven Andersson', NULL, NULL, 1, 'sven@hm.com', 'Active');

INSERT INTO USER_ (user_id, username, password_hash, role, full_name, factory_id, worker_id, buyer_id, email, status)
VALUES (6, 'worker1', '$2b$10$6WyU3pjoR3da3dEHrWAYe.bHwqz52O/JcW53amQrrP1kkjbgSgC/2', 'Worker', 'Abul Kalam', 1, 1, NULL, 'worker1@dhakagarments.com', 'Active');
...
PROMPT Temporarily disabling trg_cert_expiry_guard for seed data...
ALTER TRIGGER trg_cert_expiry_guard DISABLE;
...
PROMPT Re-enabling trg_cert_expiry_guard...
ALTER TRIGGER trg_cert_expiry_guard ENABLE;
...
-- Update the sequence values to match the static seed insertion ranges
DECLARE
  v_max_fact NUMBER;
  v_max_buy  NUMBER;
  v_max_work NUMBER;
...
BEGIN
  -- Sync FACTORY_SEQ
  SELECT MAX(factory_id) INTO v_max_fact FROM FACTORY;
  LOOP
    SELECT FACTORY_SEQ.NEXTVAL INTO v_val FROM DUAL;
    EXIT WHEN v_val >= v_max_fact;
  END LOOP;

  -- Sync BUYER_SEQ
  SELECT MAX(buyer_id) INTO v_max_buy FROM BUYER;
  LOOP
    SELECT BUYER_SEQ.NEXTVAL INTO v_val FROM DUAL;
    EXIT WHEN v_val >= v_max_buy;
  END LOOP;

  -- Sync WORKER_SEQ
  SELECT MAX(worker_id) INTO v_max_work FROM WORKER;
  LOOP
    SELECT WORKER_SEQ.NEXTVAL INTO v_val FROM DUAL;
    EXIT WHEN v_val >= v_max_work;
  END LOOP;

  -- Sync USER_SEQ
  SELECT MAX(user_id) INTO v_max_user FROM USER_;
  LOOP
    SELECT USER_SEQ.NEXTVAL INTO v_val FROM DUAL;
    EXIT WHEN v_val >= v_max_user;
  END LOOP;

  -- Sync AUDIT_SEQ
  SELECT MAX(audit_id) INTO v_max_aud FROM "AUDIT";
  LOOP
    SELECT AUDIT_SEQ.NEXTVAL INTO v_val FROM DUAL;
    EXIT WHEN v_val >= v_max_aud;
  END LOOP;

  -- Sync CERTIFICATION_SEQ
  SELECT MAX(cert_id) INTO v_max_cert FROM CERTIFICATION;
  LOOP
    SELECT CERTIFICATION_SEQ.NEXTVAL INTO v_val FROM DUAL;
    EXIT WHEN v_val >= v_max_cert;
  END LOOP;

  -- Sync SAFETY_EQUIPMENT_SEQ
  SELECT MAX(equipment_id) INTO v_max_seq FROM SAFETY_EQUIPMENT;
  LOOP
    SELECT SAFETY_EQUIPMENT_SEQ.NEXTVAL INTO v_val FROM DUAL;
    EXIT WHEN v_val >= v_max_seq;
  END LOOP;

  -- Sync GRIEVANCE_SEQ
  SELECT MAX(grievance_id) INTO v_max_seq FROM GRIEVANCE;
  LOOP
    SELECT GRIEVANCE_SEQ.NEXTVAL INTO v_val FROM DUAL;
    EXIT WHEN v_val >= v_max_seq;
  END LOOP;

  -- Sync SALARY_RECORD_SEQ
  SELECT MAX(record_id) INTO v_max_seq FROM SALARY_RECORD;
  LOOP
    SELECT SALARY_RECORD_SEQ.NEXTVAL INTO v_val FROM DUAL;
    EXIT WHEN v_val >= v_max_seq;
  END LOOP;
END;
/

-- Recalculate compliance scores across seed factories
BEGIN
  pkg_factory_mgmt.sp_update_compliance_status(1);
  pkg_factory_mgmt.sp_update_compliance_status(2);
  pkg_factory_mgmt.sp_update_compliance_status(3);
  pkg_factory_mgmt.sp_update_compliance_status(4);
  pkg_factory_mgmt.sp_update_compliance_status(5);
  pkg_factory_mgmt.sp_update_compliance_status(6);
  pkg_factory_mgmt.sp_update_compliance_status(7);
  pkg_factory_mgmt.sp_update_compliance_status(8);
  pkg_factory_mgmt.sp_update_compliance_status(9);
  pkg_factory_mgmt.sp_update_compliance_status(10);
END;
/
